...
## 🎯 <font color='#ff7f50'>Session</font>
...
## 🎯 <font color='#ff7f50'>Response</font>
...
```html
<form method="post" id="form1" action="check.jsp">
        <i>用户名</i>
        <input type="text" name="user" >
        <i>密&nbsp;&nbsp;码</i>
        <input type="password" name="pwd" >
        <input type="button" name="login" onclick="check(form1,1)" value="Login">
</form>
...
<%
    ...
    if(user!=null&&pwd!=null&&user.equals("LHS")&&pwd.equals("123456")){
        session.setAttribute("user",user);
        session.setAttribute("password",pwd);
        out.println("登陆成功，2秒后跳转到");
        response.setHeader("refresh","2;url=welcome.jsp");
    }else{
        out.println("登陆失败，2秒后跳转到login页面");
        response.setHeader("refresh","2;url=login.jsp");
    }
%>
```
